...
#import "LABStockKLineMainView.h"
...
@interface LABStockKLineMainView ()<UIGestureRecognizerDelegate>

///绘制数据源数组
@property (nonatomic, strong) NSArray<id<LABStockDataProtocol>> *drawLineModels;
///绘制数据源位置数组
@property (nonatomic, strong) NSArray<LABStockKLinePositionModel *> *drawLinePositionModels;
///K线部分
@property (nonatomic, strong) LABStockKLineContainerView *kLineView;
///成交量部分
...
@implementation LABStockKLineMainView
...
///初始化需要的子View
- (void)initSubViews {
    _kLineView = [LABStockKLineContainerView new];
...
- (void)drawRect:(CGRect)rect {
    ...
    ///绘制K线,并返回坐标
    self.drawLinePositionModels = [self.kLineView drawViewWithXPosition:xPosition
    ...
    [self.accessoryView drawViewWithXPosition:xPosition
                                  ...
                                  selectIndex:self.selectIndex];
    ///绘制时间
    __block NSMutableArray<NSValue *> *position = [NSMutableArray array];
    [self.drawLinePositionModels enumerateObjectsUsingBlock:^(LABStockKLinePositionModel *obj, NSUInteger idx, BOOL *stop) {
        [position addObject:[NSValue valueWithCGPoint:obj.accessoryPoint]];
    }];
    ...
    if (self.maskView && !self.maskView.hidden) {
        ...
        CGPoint p = self.drawLinePositionModels[index].accessoryPoint;
        ///K线绘制页面在其父类的LABStockScrollViewTopGap处开始,所以坐标还需加上这个值
...
- (UIView *)maskView {
    if (!_maskView) {
        _maskView = [LABStockKLineMaskView new];
...
- (void)event_longPressAction:(UILongPressGestureRecognizer *)longPress {
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state) {
        ...
        CGPoint p = self.drawLinePositionModels[selIndex].accessoryPoint;
        CGPoint p1 = CGPointMake(p.x, p.y+LABStockScrollViewTopGap);
        [self.maskView updateSelectModel:self.selectedModel positionPoint:p1 stockTimePositionY:CGRectGetMinY(self.timeView.frame)];
        [self.kLineView updateSelectIndex:self.selectIndex];
